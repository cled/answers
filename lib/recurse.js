...
var getLineage = function(tree){
    ...
    function traverse2(node){
        ...
        for (var key in node){
            ...
            bucket.push(key);  // document the structure as we go

            if ( !( _.isEmpty( node[key] ) ){  // if current key is not empty

                traverse2( node[key] ); // keep traversing
